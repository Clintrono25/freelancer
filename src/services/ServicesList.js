  ...
  return (
    <SafeAreaView style={styles.globalContainer}>
      ...
      <View style={[styles.cardView, {marginTop: 10, flex: 1}]}>
        <Text style={styles.homeHeadingStyle}>{Translation.serviceListExploreServices}</Text>
        {spinner ? (
          <View
            style={{
              flex: 1,
          ...
          {selectedInfo == 1 ? (
            <View>
              ...
              <RangeSlider
                ...
                gravity={'center'}
                min={lowPriceFromSetting}
                max={highPriceFromSetting}
                floatingLabel
                step={1}
                      ...
                      styles.JobDetailItemView,
                      {
                        backgroundColor:
                          ...
                          selectedLoaction.includes(item.slug)
                            ? Constant.greenColor
                            : Constant.whiteColor,
                            borderColor: selectedCategory.includes(item.slug) ||
                            selectedDeliveryTime.includes(item.slug) ||
                            selectedResponseTime.includes(item.slug) ||
                            selectedEnglishLevel.includes(item.slug) ||
                            selectedLanguage.includes(item.slug) ||
