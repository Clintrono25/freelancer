...
const JobList = ({navigation, route}) => {
  ...
  const getJobList = async () => {
    ...
    return fetch(
      Constant.BaseUrl +
        'listing/get_jobs?listing_type=search&page_number=1&minprice=1&maxprice=1000&keyword=' +
        searchVal +
        '&profile_id=' +
        userInfo.profile_id +
        '&user_id=' +
  ...
  const loadMoreData = async () => {
    ...
    return fetch(
      Constant.BaseUrl +
        'listing/get_jobs?listing_type=search&minprice=1&maxprice=1000&keyword=' +
        searchVal +
        '&profile_id=' +
        userInfo.profile_id +
        '&user_id=' +
                    ...
                    styles.JobDetailItemView,
                    {
                      backgroundColor:
                        ...
                        selectedLanguage.includes(item.slug)
                          ? Constant.greenColor
                          : Constant.whiteColor,
                          borderColor: selectedFreelancer.includes(item.slug) ||
                          // selectedHourlyRates.includes(item.slug) ||
                          selectedEnglishLevel.includes(item.slug) ||
                          selectedProjectType == item.slug ||
                          selectedIndustrialExperience.includes(item.slug) ||
